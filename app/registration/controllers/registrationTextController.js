...
RegistrationTextController.getRegistrationText = async function(req, res){
    ...
    let rows        = await registrationTextModel.find(filterQuery).skip(start).sort({ created_at: -1, status: 1 }).limit(limit).execAsync();
...
RegistrationTextController.updateRegistrationText = async function(req, res){  
    ...
    try{
        if(!errors.length){
            ...
            registration_data.central_model_name= req.body.model_name?commonHelper.removeMakeNameFromModelName(req.body.make_name, req.body.model_name):'';
